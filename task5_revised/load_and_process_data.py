...
def load_and_process_data(
    ...
    #Handle MultiIndex columns (from yfinance fresh download)
    if isinstance(df.columns, pd.MultiIndex):
    ...
    # Extract train/test date indices for plotting later
    if split_method == "date":
        train_dates = df.index[: int(len(df) * (1.0 - test_size))].to_numpy()
        test_dates = df.index[int(len(df) * (1.0 - test_size)) :].to_numpy()
    else:
        # For random splits, no chronological order return None
        train_dates = None
        test_dates = None
