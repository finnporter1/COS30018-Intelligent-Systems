...
# from yahoo_fin import stock_info as si   # original yahoo_fin import (disabled)
import yfinance as yf   # fix: switched to yfinance since yahoo_fin was unstable
...
def load_data(ticker, n_steps=50, scale=True, shuffle=True, lookup_step=1, split_by_date=True,
    ...
    if isinstance(ticker, str):
        # load it from yfinance (replaced si.get_data with yf.download)
        # fetch 1 year of daily data (copies yahoo_fin default)
        df = yf.download(ticker, period="1y", interval="1d", progress=False, auto_adjust=False)

        # yfinance can return MultiIndex columns – flatten to first level
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)

        # fix column names to match yahoo_fin style
        df.rename(columns={
            "Adj Close": "adjclose",
            "Open": "open",
            "High": "high",
    ...
    subset_cols = [c for c in (feature_columns + ['future']) if c in df.columns]
    if not subset_cols:
        raise ValueError(
